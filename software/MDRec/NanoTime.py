        ...
        if not self.__intialized:
            try:
                if tzinfo is not None:
                    ...
                    self.__timezone=pytz.utc
                if isinstance(utc_nanoseconds_since_epoch,(int,signedinteger,unsignedinteger)) and utc_nanoseconds_since_epoch>=1000:
                    ...
                    if dt.tzinfo is None:
                        ...
                        self.__UTC_Time=datetime(dt.year,dt.month,dt.day,dt.hour,dt.minute,dt.second,dt.microsecond,tzinfo=pytz.utc)
                    else:
                        utc_date=dt.astimezone(pytz.utc)
                        seconds=(datetime(utc_date.year,utc_date.month,utc_date.day,utc_date.hour,utc_date.minute,utc_date.second,tzinfo=pytz.utc)-self.__epoch).total_seconds()
                        self.__utc_value=int(seconds*1000*1000*1000+utc_date.microsecond *1000)
                        self.__UTC_Time=datetime(utc_date.year,utc_date.month,utc_date.day,utc_date.hour,utc_date.minute,utc_date.second,utc_date.microsecond,tzinfo=pytz.utc)
                else:
                    raise ValueError("Cannot Initialize: " + str(utc_nanoseconds_since_epoch) + " of type " + str(type(utc_nanoseconds_since_epoch)))
...
if __name__=="__main__":
    ...
    class TestNanoTime(unittest.TestCase):
        def setUp(self):
            ...
            self.b=NanoTime(1543838936220310003,tzinfo="Australia/Sydney")
            self.c=NanoTime(1543838936220310002,tzinfo="Australia/Sydney")
