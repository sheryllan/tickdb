...
if __name__ == '__main__':
    ...
    class IEEE80211TestCase(unittest.TestCase):
        def test_802211(self):
            ...
            self.assertTrue(ieee.type == CTL_TYPE)
            self.assertTrue(ieee.subtype == C_ACK)
            ...
            self.assertTrue(ieee.ack.dst == '\x00\x12\xf0\xb6\x1c\xa4')
        ...
        def test_80211_beacon(self):
            ...
            ieee = IEEE80211(s)
            self.assertTrue(ieee.version == 0)
            self.assertTrue(ieee.type == MGMT_TYPE)
            self.assertTrue(ieee.subtype == M_BEACON)
            self.assertTrue(ieee.to_ds == 0)
            self.assertTrue(ieee.from_ds == 0)
            self.assertTrue(ieee.pwr_mgt == 0)
            self.assertTrue(ieee.more_data == 0)
            self.assertTrue(ieee.wep == 0)
            self.assertTrue(ieee.order == 0)
            self.assertTrue(ieee.mgmt.dst == '\xff\xff\xff\xff\xff\xff')
            self.assertTrue(ieee.mgmt.src == '\x00\x26\xcb\x18\x6a\x30')
            self.assertTrue(ieee.beacon.capability == 0x3104)
            self.assertTrue(ieee.capability.privacy == 1)
            self.assertTrue(ieee.ssid.data == 'CAEN')
            self.assertTrue(ieee.rate.data == '\x82\x84\x8b\x0c\x12\x96\x18\x24')
            self.assertTrue(ieee.ds.data == '\x01')
            self.assertTrue(ieee.tim.data == '\x00\x01\x00\x00')
        ...
        def test_80211_data(self):
            ...
            ieee = IEEE80211(s)
            self.assertTrue(ieee.type == DATA_TYPE)
            self.assertTrue(ieee.subtype == D_DATA)
            self.assertTrue(ieee.data_frame.dst == '\x00\x02\xb3\xd6\x26\x3c')
            self.assertTrue(ieee.data_frame.src == '\x00\x16\x44\xb0\xae\xc6')
            self.assertTrue(ieee.data_frame.frag_seq == 0x807e)
            self.assertTrue(ieee.data == '\xaa\xaa\x03\x00\x00\x00\x08\x00\x45\x00\x00\x28\x07\x27\x40\x00\x80\x06\x1d\x39\x8d\xd4\x37\x3d\x3f\xf5\xd1\x69\xc0\x5f\x01\xbb\xb2\xd6\xef\x23\x38\x2b\x4f\x08\x50\x10\x42\x04\xac\x17\x00\x00')

            from . import llc, ip
            llc_pkt = llc.LLC(ieee.data_frame.data)
            ip_pkt = ip.IP(llc_pkt.data)
            self.assertTrue(ip_pkt.dst == '\x3f\xf5\xd1\x69')
        ...
        def test_80211_data_qos(self):
            ...
            ieee = IEEE80211(s)
            self.assertTrue(ieee.type == DATA_TYPE)
            self.assertTrue(ieee.subtype == D_QOS_DATA)
            self.assertTrue(ieee.data_frame.dst == '\x00\x26\xcb\x17\x44\xf0')
            self.assertTrue(ieee.data_frame.src == '\x00\x23\xdf\xc9\xc0\x93')
            self.assertTrue(ieee.data_frame.frag_seq == 0x207b)
            self.assertTrue(ieee.data == '')
        ...
        def test_data_ds(self):
            ...
            ieee = IEEE80211(s)
            self.assertTrue(ieee.type == DATA_TYPE)
            self.assertTrue(ieee.to_ds == 1)
            self.assertTrue(ieee.from_ds == 1)
            self.assertTrue(ieee.data_frame.sa == '\x00\x26\xcb\x18\x73\x50')
            self.assertTrue(ieee.data_frame.src == '\x00\x26\xcb\x18\x73\x50')
            self.assertTrue(ieee.data_frame.dst == '\x01\x0b\x85\x00\x00\x00')
            self.assertTrue(ieee.data_frame.da == '\x01\x0b\x85\x00\x00\x00')
            ...
            self.assertTrue(ieee.to_ds == 1)
            self.assertTrue(ieee.from_ds == 0)
            self.assertTrue(ieee.data_frame.bssid == '\x00\x26\xcb\x17\x48\xc1')
            self.assertTrue(ieee.data_frame.src == '\x00\x24\x2c\xe7\xfe\x8a')
            self.assertTrue(ieee.data_frame.dst == '\xff\xff\xff\xff\xff\xff')
            ...
            self.assertTrue(ieee.to_ds == 0)
            self.assertTrue(ieee.from_ds == 1)
            self.assertTrue(ieee.data_frame.bssid == '\x00\x1f\x33\x39\x75\x44')
            self.assertTrue(ieee.data_frame.src == '\x00\x1f\x33\x39\x75\x44')
            self.assertTrue(ieee.data_frame.dst == '\x00\x02\x44\xac\x27\x70')
