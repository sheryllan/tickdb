...
class FrontMonthCheckTask(enriched.CheckTask):
    ...
    def get_bar_by_contract(self, time_start, time_end, contract, include_to, **kwargs):
        field_dict = contract[[TagsC.PRODUCT, TagsC.TYPE, FieldsC.EXPIRY]].to_dict()
        ...
        data_params = {**kwargs, **field_dict,
                       ...
                       self.accessor.INCLUDE_TO: include_to}
        return self.get_bar_data(**data_params, empty=empty_data)

    def check_contracts(self, contracts, check_xmls, **kwargs):
        dtto = self.args.dtto
        xmls = check_xmls
        for (time_start, time_end), row in contracts:
            try:
                data = self.get_bar_by_contract(time_start, time_end, row, time_end == dtto, **kwargs)
